...
async def handle_request(client_socket, session) -> None:
    ...
    if method == "GET":
        ...
        if "RSS_name" in request_data and "RSS_url" in request_data:
            ...
            # splits form_data into ["remove_feed","name+any+spaces"]
            form_data = form_data.split("=")[1]
...
def format_url(url: str):
    print(url)
    # Splits from the first '=', outputting https%3A%2F%2Ffeeds.megaphone.fm%2Fnewheights
    url = url.split("=", 1)
    url = url[1].strip().replace("%3A", ":").replace(
        "%2F", "/").replace("%2f", "/").replace("%2C", " ")
